...
static void printHelp() {
    ...
    Serial.println("r[ead] a    - read byte from address");
    Serial.println("w[rite] a v - write byte value to address");
    Serial.println("d[ump] a n  - dump n bytes from address");
...
static void performStepCommand(bool is_inst_step) {
    ...
    for(long i=0; i<n; ++i) {
        if(!controlIsClaimed()) {
            Serial.println("aborting: processor not halted");
            return;
        }
...
static SerialState processCommand() {
    ...
    if(n_tokens == 0) {
        ...
        int should_halt = !controlIsHalted();
        controlSetHalt(should_halt);
        Serial.print("halt ");
        Serial.println(should_halt ? "on" : "off");
    } else if(strprefixeq(cmd, "print") && (n_tokens == 1)) {
        // print current state
        ControlBusState read_state;
        read_state = controlUpdateBus(read_state);
        Serial.print("A: $");
        Serial.print(read_state.address, HEX);
        Serial.print(" D: $");
        Serial.print(read_state.data, HEX);
        Serial.println("");
        Serial.print("SYNC: ");
        Serial.println(read_state.ro_control.flags.sync);
